...
export function ResultPanel({ onOpenParticipantInput }: ResultPanelProps) {
  ...
  const matchingCellIds = useMemo(() => {
    ...
    return new Set(
      assignments
        .filter((a) =>
          query
            ? a.participant?.displayName.toLowerCase().includes(query)
            : false,
        )
        .map((a) => a.cellId),
    );
  ...
  const redrawButtonLabel =
    selectedParticipantsForRedraw.length > 0
  ...
  return (
    <Card
      radius="lg"
      className="min-h-0 h-fit max-h-full overflow-hidden grid grid-rows-[auto_1fr] max-[1280px]:max-h-none print:block print:border-0 print:shadow-none"
    >
      <Stack gap="sm">
        ...
        {hasAssignments ? (
          <>
            <Card radius="md" p="xs" className="bg-surface-warm print:hidden">
              <Stack gap={6}>
                <Group justify="space-between" wrap="wrap" gap="xs">
                  <Text size="xs" c="dimmed">
                    마지막 결과: {updatedAtLabel}
                  </Text>
                  <Text size="xs" c="dimmed">
            ...
            >
              <Group gap="xs">
                ...
                <Tooltip
                  label="선택한 학생들만 다시 배정합니다 (고정석 제외)"
                  withArrow
                >
                  <Button
            ...
            {isRedrawPickerOpen && (
              <Card radius="md" p="sm" className="bg-surface-warm print:hidden">
                <Stack gap="sm">
                  ...
                  <Button
                    ...
                    disabled={
                      isDrawing || selectedParticipantsForRedraw.length === 0
                    }
                  >
                    {redrawButtonLabel}
                  </Button>
            ...
            <div className="min-h-0 overflow-auto pr-1 print:overflow-visible print:pr-0">
              ...
              >
                {seatConfig.layout.cells.map((cell, index) => {
                  ...
                  const primaryLabel =
                    drawSettings.continuousNumbering && seatNumber
                      ? `${seatNumber}번`
                      : cell.label;
                  ...
                  return (
                    ...
                    >
                      ...
                      <Text
                        fw={hasParticipant ? 600 : 500}
                        size="sm"
                        c={hasParticipant ? undefined : "dimmed"}
                      >
                        {assignment?.participant?.displayName ?? "빈자리"}
                      </Text>
                      <Text size="xs" c="dimmed">
                        {drawSettings.continuousNumbering
                          ? `칸 ${cell.label}`
                          : `${seatNumber}번 자리`}
                        {fixedSeat ? " · 고정석" : ""}
                      </Text>
          ...
          >
            <Stack align="center" gap="sm">
              ...
              <Text c="dimmed">
                {isDesktop ? "왼쪽" : "위"} 패널에서 명단을 입력하고 좌석을
                확인한 뒤,{" "}
                <Text span fw={700}>
                  자리 뽑기
                </Text>
